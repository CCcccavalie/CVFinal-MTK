...
class GC_NET(nn.Module):
    def __init__(self,block,block_3d,num_block,height,width,maxdisp):
        ...
        self.in_planes=32
        #first two conv2d
        self.conv0=nn.Conv2d(1,32,5,2,2)
    ...
    def cost_volume(self,imgl,imgr):
        ...
        xx = xx.view(1, self.maxdisp, 64, int(self.height / 2), int(self.width / 2) + self.maxdisp) #xx.view(1, self.maxdisp, 64, int(self.height / 2), int(self.width / 2) + self.maxdisp)
